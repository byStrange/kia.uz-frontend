...
defineProps<{ title?: string, subtitle?: string, direction?: string }>() 
</script>
<template>
  <MoleculeSection ref="section" :section-title="title" :subtitle="subtitle"
    class="2xl:grid 2xl:grid-cols-12 2xl:container 2xl:gap-x-grid-12-gap " align="left" :class="{
      '2xl:rtl bg-background': direction === 'left',
    }">
    <template #title="{ sectionTitle, sectionTitleClass, subtitleClass, subtitle }">
      <div class="container split-left" :class="{
      ...
      <div :class="{
        'mt-10 col-start-7 col-span-6 2xl:mt-0 split-right': true,
        ltr: direction === 'left',
      }">
        <slot name="right" />
      </div>

  ...
  <MoleculeSection :section-title="t('index.models_of_kia')" class="bg-background">
    <div class="2xl:container">
      <ClientOnly>
        <Swiper :initial-slide="6" :modules="[Controller]" :controller="{ control: modelsSwiper }"
          ...
          <template #container-start>
            <MoleculeButtonCarousel position="left" mode="free" :hide="activeModelIndex === 0" size="sm"
              @click="slidePrev" />
          ...
          <SwiperSlide v-for="model in models" :key="model.name" class="!w-fit">
            <div :class="[
              ...
              <img loading="lazy" :src="safe(model.main_image)" class="h-[40px] w-[84px] md:h-[52px] md:w-[108px]" />
      ...
      <div class="mt-4 md:mt-8">
        <ClientOnly>
          <Swiper :modules="[Controller]" :controller="{ control: modelsThumbSwiper }" :slides-per-view="1"
            ...
            <SwiperSlide v-for="model in models" :key="model.name" class="container 2xl:px-0">
              <div data-label="card">
                <div data-label="image wrapper">
                  <img loading="lazy" :src="safe(model.main_image)"

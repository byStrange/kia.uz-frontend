        ...
        <Swiper :modules="[Controller]" :controller="{ control: modelsSwiper }" slides-per-view="auto"
          ...
          <SwiperSlide v-for="model in models" :key="model.name" class="!w-fit">
            <div :class="[
              ...
              <UIElectroCarIcon v-if="model.electric" class="absolute right-1 top-2" />

              <img loading="lazy" :src="model.images['108x52']" class="h-[40px] w-[84px] md:h-[52px] md:w-[108px]" />
            ...
            <SwiperSlide v-for="model in models" :key="model.name" class="container 2xl:px-0">
              <div data-label="card">
                <div data-label="image wrapper">
                  <img loading="lazy" :src="model.images['742x334']"

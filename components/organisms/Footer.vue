  ...
  <footer class="bg-primary py-[60px]">
    <div class="container">
      ...
      <div aria-label="Footer menu">
        ...
        <div class="md:md:mt-12 md:grid md:grid-cols-6 md:gap-9 2xl:grid-cols-12">
          <div class="md:col-span-4 2xl:col-span-8">
            <div
              ...
              <div class="hidden flex-col md:flex md:gap-5">
                <NuxtLinkLocale v-for="item in footerLinks.value.filter(
                  (i) => !i.children?.length,
                )" :key="item.label" :to="item.to" class="font-semibold text-white">
                  {{ $t(item.label) }}
                </NuxtLinkLocale>
              </div>

              <UIFooterMenuItem v-for="item in footerLinks.value" :key="item.label" :item="item" />
            </div>
          </div>
          ...
          <div class="py-8 md:py-0 md:col-span-2 2xl:col-start-10">
            <div class="space-y-6">
              <div class="space-y-2.5">
                <h3 class="text-sm+ text-disabled">{{ $t('menu.kia_hotline') }}</h3>
        ...
        <div class="grid-rows-1 space-y-7.5 md:mt-12 2xl:grid 2xl:grid-cols-12 2xl:space-y-0">
          <div class="order-2 2xl:col-span-3 2xl:col-start-10">
            <AtomButton :label="$t('footer.feedback')" mode="full" class="md:!w-full" color="primaryDark"

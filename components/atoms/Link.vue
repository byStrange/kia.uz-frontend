...
const button = tv({
  base: 'inline-flex items-center justify-center whitespace-nowrap transition-all outline-none font-semibold text-base+ transition-colors',
  variants: {
    color: {
      unstyled: '',
...
<template>
  <NuxtLink :to="locale(to)" :class="button({ color, size: { initial: 'sm', md: 'md' }, mode, corner })">
    <slot>{{ label }}</slot>
  </NuxtLink>
</template>

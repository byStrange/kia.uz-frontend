  ...
  <UISafeAreaView>
    <UIContainer>
      ...
      <div
        ...
        <div v-for="model in models.slice(0, 4)" :key="model.name">
          <img :src="src('0b0e228e-8e0e-4f06-b1b6-4974f45d1d0e')" alt="" />
          <div class="mt-5">
            <h2 class="text-primary font-semibold text-base md:text-lg 2xl:text-2xl">
              {{ model.name }}
            </h2>
            <button class="flex gap-x-2 mt-2 cursor-pointer">
      ...
      <div class="pb-12 md:grid md:grid-cols-2 rtl">
        ...
        <div
          ...
          <AtomButton mode="full" color="secondary" label="Подробнее" class="mt-6 !text-base+" />
        </div>
      </div>
    </UIContainer>
  </UISafeAreaView>
